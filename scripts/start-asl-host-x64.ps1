...
$hostDir = Join-Path $AslHostRoot "x64"
$exePath = Join-Path $hostDir "TournamentTimer.AslHost.PoC.exe"

if (-not (Test-Path -LiteralPath $exePath)) {
    Write-Host "ASL Host x64 runtime was not found: $exePath" -ForegroundColor Yellow
...
if ($Catalog) {
    ...
    Write-Host "Starting ASL Host x64 catalog GUI"
...
Write-Host "Starting ASL Host x64"
Write-Host "Host:   $exePath"
